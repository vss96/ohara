...
mod batch_tests {
    ...
    fn schema(c: &Connection) {
        ...
        .expect("invariant: in-memory schema MUST create cleanly");
    }
    ...
    fn insert(c: &Connection, hunk_id: i64, name: &str, kind: AttributionKind) {
        ...
        .expect("invariant: test fixture insert MUST succeed");
    }

    #[test]
    fn get_for_hunks_groups_results_by_hunk_id_in_a_single_query() {
        let c = Connection::open_in_memory().expect("invariant: in-memory open MUST succeed");
        schema(&c);
        ...
        let got = get_for_hunks(&c, &[10_i64, 11, 12])
            .expect("invariant: batch lookup MUST succeed against the test fixture");
    ...
    fn get_for_hunks_returns_empty_map_for_empty_input() {
        let c = Connection::open_in_memory().expect("invariant: in-memory open MUST succeed");
        schema(&c);
        let got = get_for_hunks(&c, &[])
            .expect("invariant: empty input MUST succeed without SQL roundtrip");
