...
fn collect_related_commits<'a>(
    storage: &'a dyn Storage,
    repo_id: &'a RepoId,
    file: &'a str,
    hits: &'a [ExplainHit],
) -> impl std::future::Future<Output = Result<(Vec<RelatedCommit>, Option<String>)>> + 'a {
    hydrator::collect_related_commits(storage, repo_id, file, hits)
}
